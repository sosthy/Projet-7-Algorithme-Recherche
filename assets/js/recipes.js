...
const APP = {
    ...
    donneesFiltresSelected: {
        ingredients: new Set([]),
        ustensils: new Set([]),
        appareils: new Set([])
    },
    // Resultat de la recherche à afficher
    resultatRechercheRecette: new Set([]),
    getIntersectionByProperty: (sets, property) => 
    {
        if (sets.length === 0) return new Set();
        
        // Commencer avec les éléments du premier ensemble
        let intersection = new Set(sets[0]);

        for (const set of sets) {
            intersection = new Set([...intersection].filter(item1 =>
                [...set].some(item2 => item1[property] === item2[property])
            ));
        }
